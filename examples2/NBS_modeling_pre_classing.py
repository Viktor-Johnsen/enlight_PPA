...
class NBS_Model:
    ...
    '''
    def __init__(self,
        S_R : float,  # Minimum PPA strike price
        S_U : float,  # Maximum PPA strike price
        M_R : float,  # BL: Minimum baseload volume
        M_U : float,  # BL: Maximum baseload volume
        gamma_R : float, # PaP: Minimum PPA capacity share volume
        gamma_U : float, # PaP: Minimum PPA capacity share volume
        beta_D : float,  # CVaR: Risk-aversion level of developer
        beta_O : float,  # CVaR: Risk-aversion level of off-taker
        alpha : float,  # CVaR: Tail of interest for CVaR
        PPA_profile: str = 'BL',  # Type of PPA profile ('BL' or 'PaP')
        PPA_settlement : str = 'current',  # Settlement mechanism ('current' or 'future')
        PPA_BL_compliance : float = 0, # indicates the enforced compliance of the producer: meaning the % of PPA volume where the producer has to match the BL volume on an hourly basis
        add_batt: bool = None,
        ...
        self.PPA_settlement = PPA_settlement  # 'future' = "coupled PaP" or "compliance BL"
        if not (1 >= PPA_BL_compliance >= 0):
            raise Exception(f"The chosen level of PPA BL compliance is not valid: {PPA_BL_compliance}. It should be between [0,1].")
        else:
            self.PPA_BL_compliance = PPA_BL_compliance
    ...
    def compute_disagreement_points(self) -> None:
        ...
        self.d_O = (
            (1 - beta_O) * sum([PI_O_w[w] * self.PROB_w[w] for w in range(self.W)])
            + beta_O * (VAR_O - 1/(1-alpha) * sum([ETA_O_w[w] * self.PROB_w[w] for w in range(self.W) if ETA_O_w[w] >= 0]))
        )
        ...
        if self.PPA_profile == 'BL':  # -> overwrite d_D but keep d_O
    ...
    def build_profile_vars_constrs(self) -> None:
        ...
        if self.PPA_profile == 'BL':
            ...
            if self.PPA_settlement == 'future':
                # Power offered in DA outside of the PPA
                self.p_DA = self.model.addMVar(shape=(self.T, self.W), lb=0, vtype=GRB.CONTINUOUS, name="p_DA")
        ...
        if self.PPA_profile == 'BL':
            ...
            if self.PPA_BL_compliance > 0:
                ...
                self.model.addConstrs(
                    (gp.quicksum(self.v_min[t, w] for t in range(self.T))
                     >= self.PPA_BL_compliance * self.T * self.M
                     for w in range(self.W)),
                     name="c_BL_compliance")

        elif self.PPA_profile == 'PaP':
            if self.PPA_settlement == 'current':
            ...
            elif self.PPA_settlement == 'future':
                # Restrict DA offers
                self.model.addConstrs((self.p_DA[t, w]
                                       <=
                                       (1 - self.gamma) * self.P_fore_w[t, w]
                                       for t in range(self.T) for w in range(self.W)),
                                       name="c_p_DA")
                ...
                self.model.addConstrs((self.y_D[w] ==
                                    gp.quicksum(
                                        ...
                                        self.p_DA[t, w] * self.lambda_DA_w[t, w]  # DA revenues
                                        # self.gamma is implicit in p_PPA because of
                                        #   the upper bound gamma*P_fore
                                        + self.p_PPA[t, w] * self.S # PaP PPA revenues
                                        
                                        # We need to pay back the BRP for imbalances between forecast and realized production, however,
                                        #   this calculation must be paid only in hours of non-negative prices. Otherwise the BRP pays the developer money for creating an imbalance...
                                        - self.lambda_DA_w[t, w]
                                            # * (self.gamma * self.P_fore_w[t, w] - self.p_PPA[t, w])
                                            * (self.gamma * self.P_DA_w[t, w] - self.p_PPA[t, w])
                                        for t in range(self.T)
                                    ) for w in range(self.W)), name='c_yD_link_PaP')
                self.model.addConstrs((self.y_O[w] ==
                                    gp.quicksum(
                ...
                raise Exception(f"{self.PPA_settlement} is not a valid settlement type. Please choose 'current' or 'future'." )
...
if __name__ == "__main__":
    ...
    # Profile type
    PPA_profile = 'BL' # -> BATTERY only implemented in d_D not in u_D...
    PPA_settlement = 'current' # -> no effect on BL yet!
    PPA_BL_compliance = 0.9

    # Define ranges for betas
    beta_D_list = np.round(np.arange(0.0, 0.51, 0.25), 2)  # avoid floating point issues
    beta_O_list = np.round(np.arange(0.0, 0.51, 0.25), 2)  # avoid floating point issues
    ...
    for beta_O in beta_O_list:
        ...
        for beta_D in beta_D_list:        
            ...
            nbs_model = NBS_Model(
                S_R=S_R, S_U=S_U,  # PPA price
                M_R=M_R, M_U=M_U,  # BL volume
                gamma_R=gamma_R, gamma_U=gamma_U,  # PaP volume
                beta_D=beta_D, beta_O=beta_O, alpha=alpha,
                PPA_profile=PPA_profile,  # BL or PaP
                PPA_settlement=PPA_settlement,
                PPA_BL_compliance=PPA_BL_compliance,
            ...
            if nbs_model.model.status == GRB.OPTIMAL:
                ...
                elif nbs_model.PPA_profile == 'PaP':
    ...
    beta_D_grid, beta_O_grid = np.meshgrid(beta_D_list, beta_O_list)
    S_vals = np.array([[results_S[bO][bD] for bD in beta_D_list] for bO in beta_O_list])

    if PPA_profile == 'BL':
        volume_vals = np.array([[results_volume[bO][bD] for bD in beta_D_list] for bO in beta_O_list])
    ...
    im1 = axs[0].imshow(S_vals, origin='lower', cmap='coolwarm',
                        extent=[min(beta_D_list), max(beta_D_list), min(beta_O_list), max(beta_O_list)], aspect='auto')
    ...
    im2 = axs[1].imshow(volume_vals, origin='lower', cmap='viridis',
                        extent=[min(beta_D_list), max(beta_D_list), min(beta_O_list), max(beta_O_list)], aspect='auto')
    axs[1].set_title(fr"{PPA_profile} volume {"(M)" if PPA_profile == 'BL' else "($\gamma$)"}")
    ...
    ##### PLOT RESULTS (and data) FOR EXAMPLE RISK-AVERSION LEVELS #####
    # Results: PPA price and volume
    # nput data: load profile, generation and price forecasts
    for bO in beta_O_list:
        for bD in beta_D_list:
            if not (np.isnan(results_S[bO][bD])  and np.isnan(results_volume[bO][bD])): # choose levels that results in an agreement
    ...
    S_X = results_S[beta_O_chosen][beta_D_chosen]
    low_perc = 10
    high_perc = 90
    ...
    nbs_ = models[beta_O_list[0]][beta_D_list[0]]  # any instance can be used to retrieve the data
    ...
    if PPA_profile == 'BL':
        M_X = results_volume[beta_O_chosen][beta_D_chosen]
        ax[0].axhline(M_X, color='k', linestyle='-.', alpha=0.7,
                    label='Agreed BL volume $M$')
    elif PPA_profile == 'PaP':
        gamma_X = results_volume[beta_O_chosen][beta_D_chosen]
    ...
    # Pure DA revenues
    PI_D_w = [models[beta_O_chosen][beta_D_chosen].PI_D_w[w] for w in range(nbs_.W)]
    PI_O_w = [models[beta_O_chosen][beta_D_chosen].PI_O_w[w] for w in range(nbs_.W)]
    # VaR before PPA
    VaR_D = models[beta_O_chosen][beta_D_chosen].VAR_D
    VaR_O = models[beta_O_chosen][beta_D_chosen].VAR_O
    # Expected DA + PPA revenues
    PI_D_w_NBS = [models[beta_O_chosen][beta_D_chosen].y_D[w].X for w in range(nbs_.W)]
    PI_O_w_NBS = [models[beta_O_chosen][beta_D_chosen].y_O[w].X for w in range(nbs_.W)]
    # VaR after PPA
    zeta_D = models[beta_O_chosen][beta_D_chosen].zeta_D.X
    zeta_O = models[beta_O_chosen][beta_D_chosen].zeta_O.X
    ...
    if d.PPA_settlement == 'future' and d.PPA_profile == 'PaP':
        for i in range(d.W)[:4]:
            ...
            ax.plot(d.p_DA.X[:,i] + d.p_PPA.X[:,i], ls=':', alpha=.5, label=r"$p^{DA} + p^{PPA}$")
            ...
            ax.plot(d.p_DA.X[:,i], alpha=.5, ls='--', label=r"$p^{DA}$")
    ...
    d=models[0.50][0.25]
    w=3
    plt.plot(d.P_fore_w[:, w], label="P_fore")
    plt.plot((d.p_DA.X + d.y_ch.X)[: ,w], label="p_DA + y_ch", ls='--')
    plt.plot(d.p_DA.X[: ,w], label="p_DA", ls='--', alpha=.5)
    if d.PPA_BL_compliance > 0:
        plt.plot(d.v_min.X[: ,w], label="v_min", c='r', alpha=.5)
    plt.plot(d.SOC.X[: ,w], label="SOC", ls=':', alpha=0.3)
